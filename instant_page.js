...
let _chromiumMajorVersionInUserAgent = null
  ...
  , _persistentPreloads = new Map() // Track persistent preloads
...
function preload(url, fetchPriority = 'auto') {
  ...
  // Create a persistent preload that won't be aborted
  createPersistentPreload(url, fetchPriority);
...
// Create a persistent preload using XHR that won't be aborted
function createPersistentPreload(url, fetchPriority = 'auto') {
  // Don't preload the same URL twice
  if (_persistentPreloads.has(url)) {
    return;
  }
  
  try {
    // Use XMLHttpRequest which is less likely to be aborted
    const xhr = new XMLHttpRequest();
    xhr.open('GET', url, true);
    xhr.responseType = 'document';
    xhr.timeout = 30000; // 30 seconds timeout
    
    // Set a low priority to not interfere with other resources
    if (fetchPriority !== 'high') {
      xhr.setRequestHeader('Priority', 'u=1');
    }
    
    xhr.send();
    _persistentPreloads.set(url, xhr);
  } catch (e) {
